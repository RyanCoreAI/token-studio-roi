...
test('resolveLaunchCwd expands Windows 8.3 temp paths before starting Vite', () => {
  const shortPackageRoot = 'C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\token-studio-npx-smoke\\run\\node_modules\\token-studio';
  const longPackageRoot = 'C:\\Users\\runneradmin\\AppData\\Local\\Temp\\token-studio-npx-smoke\\run\\node_modules\\token-studio';
...
test('resolveLaunchCwd falls back to the package root when realpath is unavailable', () => {
  const packageRoot = join(tmpdir(), `token-studio-runtime-${Date.now()}-${Math.random().toString(16).slice(2)}`);
