...
test('token-studio collect wrapper defaults to dry-run-only writes when requested', async () => {
  const fixture = createCollectorFixture();
  try {
    const result = await runNode([
      'src/cli.mjs',
...
function createCollectorFixture() {
  ...
  mkdirSync(join(claudeRoot, 'projects', 'token-studio'), { recursive: true });
  mkdirSync(join(codexHome, 'sessions', '2026', '06', '17'), { recursive: true });
  mkdirSync(cursorStorage, { recursive: true });

  writeFileSync(join(claudeRoot, 'projects', 'token-studio', 'claude-session.jsonl'), [
    JSON.stringify({
      type: 'assistant',
  ...
  writeFileSync(join(claudeRoot, 'projects', 'token-studio', 'claude-session-2.jsonl'), [
    JSON.stringify({
      type: 'assistant',
...
function tempDir() {
  return mkdtempSync(join(tmpdir(), 'token-studio-collect-'));
}
