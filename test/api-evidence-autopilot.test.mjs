...
test('evidence suggestion API builds and applies selected high-confidence evidence', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'token-studio-api-evidence-'));
  ...
  try {
    ...
    assert.equal(JSON.stringify(planResponse).includes('D:\\HighROIProjects\\token-studio'), false);
    ...
    assert.equal(data.sessions[0].projectAlias, 'Token Studio');
...
function seedDb(dbPath) {
  ...
  try {
    ...
    upsertSessionOutput(db, {
      ...
      source: 'Codex CLI',
      sessionId: 'local:codex:D:\\HighROIProjects\\token-studio:gpt-5.5',
      outputUrl: 'https://github.com/RyanCoreAI/token-studio-roi/commit/abcdef1234567890',
      outputLabel: 'token-studio abcdef12',
      outputType: 'commit'
    });
  } finally {
    db.close();
