...
test('evidence autopilot turns alias rules and git commit URLs into applicable evidence', () => {
  const plan = buildEvidenceAutopilotPlan({
    ...
    projectAliasRules: [{
      enabled: true,
      matchType: 'prefix',
      pattern: 'D:\\HighROIProjects\\token-studio',
    ...
    gitCandidatesBySession: new Map([[
      ...
      {
        repoName: 'token-studio',
        remoteHost: 'github.com',
        commitHash: 'abcdef1234567890',
        commitAt: '2026-06-18T10:00:00.000Z',
        commitUrl: 'https://github.com/RyanCoreAI/token-studio-roi/commit/abcdef1234567890',
  ...
  assert.equal(plan.suggestions.find(item => item.kind === 'annotation').suggestedValues.projectAlias, 'Token Studio');
});
...
test('evidence suggestions do not expose full local project paths', () => {
  const plan = buildEvidenceAutopilotPlan({
    sessions: [{
      ...baseSession,
      projectPath: 'D:\\HighROIProjects\\private-client\\token-studio',
      sessionId: 'local:codex:D:\\HighROIProjects\\private-client\\token-studio:gpt-5.5'
    }],
    period: 'all',
    scanGit: false
  });
