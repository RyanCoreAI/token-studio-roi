...
assert.match(source, /TOKEN_STUDIO_LIVE_COLLECT_INTERVAL_SECONDS/, 'desktop should use the live collect interval override');
assert.match(source, /isTokenStudioApi/, 'desktop must verify the local API identifies as Token Studio before reuse');
assert.match(source, /isTokenStudioUi/, 'desktop must verify the local UI identifies as Token Studio before reuse');
assert.match(source, /isTokenStudioUiApi/, 'desktop must verify the local UI can proxy API requests before reuse');
