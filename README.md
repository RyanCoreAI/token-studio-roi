...
Token Studio ROI 的 v4.4 重点不是再堆一个 token meter，而是把统计结果转成可执行决策，并降低普通用户把外部结构化用量接进 ROI 复盘的门槛：
...
- ccusage Import：`token-studio import-usage --format=ccusage-json` 导入结构化 JSON，不保存正文，不采用第三方估算成本。
