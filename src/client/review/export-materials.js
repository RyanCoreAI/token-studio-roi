...
export function buildProfessionalEvidencePack({
  ...
  return [
    '# Token Studio ROI Review Evidence',
    '',
    ...
    table(
      ['覆盖方式', '数量', '说明'],
      [
        ['原生可信采集', formatInt(coverage.nativeTrusted), 'Token Studio 直接读取可靠 token 字段。'],
...
export function buildTechnicalBlogDraft({
  ...
  return [
    '# 我为什么做 Token Studio ROI：把 AI 编程 token 变成可复盘的工作证据',
    '',
    '## 标题候选',
    '',
    '1. 从 token 看板到 ROI 复盘：我如何管理 AI 编程投入',
    '2. Token Studio ROI：一个本地隐私优先的 AI 编程成本与产出复盘工具',
    '3. 不只统计 token：把 AI 编程消耗连接到项目、产出和模型策略',
    '',
    '## 摘要',
    '',
    `本期样本包含 ${formatInt(sessions.length)} 个 session、${compactCN(totals.total || totals.totalTokens || 0)} tokens，官方价换算为 ${money(totals.cost || totals.costUSD || 0)}。Token Studio ROI 的目标不是生成供应商账单，而是回答三个更适合个人复盘的问题：token 花在哪、产生了什么证据、下周怎么用更少 token 做更高价值的事。`,
    '',
    '## 背景问题',
    '',
    ...
    '',
    '因此我把 Token Studio ROI 定位为本地复盘系统，而不是另一个 token meter。它先确认数据可信度，再把 token 转成项目、任务、阶段、价值和产出证据，最后输出模型策略和行动清单。',
    '',
    '## 方案设计',
    '',
    ...
    '- Savings Simulator：只对有官方公开美元价的模型做节省模拟，未定价模型不参与美元结论。',
    ...
    '## 局限',
    '',
    'Token Studio ROI 不能恢复已经被上游删除或从未记录 token 字段的历史；也不能自动判断真实产出质量。它能做的是把仍存在于本机的结构化 token 元数据变成可复盘证据，并把不确定性明确标出来。',
    '',
    '## 下一步',
    '',
...
export function buildResumeAndInterviewPack({
  ...
  return [
    '# Token Studio ROI Resume / Interview Pack',
    '',
    '## 中文简历版',
    '',
    `- 设计并实现本地优先的 AI 编程 ROI 复盘工具 Token Studio ROI，将 ${sessionPhrase} 和 ${totalPhrase} 连接到项目、任务、阶段、产出证据和模型策略。`,
    ...
    '',
    `- Built Token Studio ROI, a local-first AI coding ROI review system that connects ${sessionPhrase}, ${totalPhrase}, project attribution, output evidence, and model strategy without storing conversation content.`,
...
function money(value) {
  return U.fmtUS.format(Number(value || 0));
}
