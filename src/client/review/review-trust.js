export function buildReviewTrustState(meta = {}) {
  ...
  if (meta.demoMode || runtime.demoMode || dataMode.id === 'demo') {
    return {
      ...
      action: '运行 npx token-studio 进入真实本地采集模式。'
    };
  }
  ...
  if (!hasRuntime) {
    return {
      ...
      facts: [
        ...
        { label: '建议', value: '重启 npx token-studio@latest' }
      ],
      action: '关闭旧 Node/Vite 进程后重新运行 npx token-studio@latest。'
    };
  }
  ...
  if (dataMode.id === 'real-event-verified' && tokenEvents > 0) {
    return {
      ...
      tone: 'ok',
      title: '这是可信的真实本地 token 数据',
      summary: '当前 SQLite 已有 event 级 token 记录，并且最近 coverage gate 或 collect run 可验证。美元值是官方公开价换算，不是供应商账单。',
      trusted: true,
      facts: [
        { label: 'Session', value: formatInt(sessions) },
        { label: 'Token events', value: formatInt(tokenEvents) },
        { label: 'Coverage', value: coverageGate.status === 'passed' ? '已通过' : '有 collect run' }
      ],
      latestRun,
  ...
  if (dataMode.id === 'real-aggregate-only' || tokenEvents === 0 && (sessions > 0 || dailyRows > 0)) {
    return {
      ...
      action: '运行 npx token-studio 重新走 coverage -> collect -> event verified。'
    };
  }
  ...
  return {
    ...
    action: '运行 npx token-studio，或 npx token-studio demo 查看合成数据。'
  };
}
