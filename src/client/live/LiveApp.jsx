...
export function LiveApp() {
  ...
  const generated = useMemo(() => formatCstDateTime(snapshot?.generatedAt), [snapshot?.generatedAt]);
  const statuslineCommand = `npx token-studio statusline --format=text --window-minutes=${PULSE_WINDOW_MINUTES}`;
    ...
    <main className={`live-shell ${isDesktopPulse ? 'desktop-pulse' : 'browser-live'}`}>
      <nav className="pulse-nav">
        <div className="pulse-brand">
          <span className="pulse-logo" aria-hidden="true">TS</span>
          <strong>Token Studio Pulse</strong>
      ...
      {(error || refreshError) && (
        <section className="pulse-error">
          ...
          <span>这通常表示 UI 代理没有连上本地 API。请重新运行 Token Studio，或在 /trust 查看服务状态。</span>
        </section>
      )}
      ...
      <section className="pulse-kpi-grid" aria-label="近24小时核心指标">
        ...
        <KpiCard accent="green" label="近24小时官方价" value={`$${formatMoney(totals.costUSD)}`} detail="官方公开价换算，不是账单" trend={timeline.map(row => row.costUSD)} />
      ...
      <footer className="pulse-footer">
        ...
        <span>最新事件：{formatCstDateTime(snapshot?.latestEventAt)}</span>
...
function FreshnessNotice({ snapshot }) {
  ...
  return (
    <section className={`pulse-freshness status-${freshness}`} aria-label="实时数据新鲜度">
      ...
      <small>上次刷新：{formatCstDateTime(snapshot.latestCollectionRunAt)} · 最新事件：{formatCstDateTime(snapshot.latestEventAt)}</small>
    </section>
  );
}
...
function TrendChart({ data }) {
  ...
  return (
    <svg className="trend-chart" viewBox="0 0 900 260" role="img" aria-label="24小时 token 和官方价趋势">
      <defs>
      ...
      {[0, 1, 2, 3, 4].map(index => <line key={`h-${index}`} x1="44" x2="876" y1={42 + index * 42} y2={42 + index * 42} className="chart-grid"/>)}
      {chart.yTicks.map(tick => (
        <g key={`tick-${tick.y}`}>
          <text x="38" y={tick.y + 4} className="chart-axis" textAnchor="end">{tick.tokens}</text>
          <text x="888" y={tick.y + 4} className="chart-axis chart-axis-right" textAnchor="start">{tick.cost}</text>
        </g>
      ))}
      {chart.labels.map(label => (
        <text key={label.x} x={label.x} y="244" className="chart-label">{label.text}</text>
      ))}
