...
const nodeBin = process.env.TOKEN_STUDIO_NODE || 'node';
const defaultApiPort = Number(process.env.TOKEN_STUDIO_DESKTOP_API_PORT || 4173);
const defaultUiPort = Number(process.env.TOKEN_STUDIO_DESKTOP_UI_PORT || 5173);
const windowTitle = 'Token Studio Pulse';
...
function createErrorWindow(error) {
  mainWindow = new BrowserWindow({
    width: 420,
    height: 260,
    title: 'Token Studio Pulse startup error',
    backgroundColor: '#05070d',
    autoHideMenuBar: true,
    webPreferences: {
      contextIsolation: true,
  ...
  mainWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(`
    <main style="font-family:system-ui,sans-serif;background:#05070d;color:#eef7ff;min-height:100vh;padding:24px">
      <p style="color:#35f4ff;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase">Token Studio Pulse</p>
      <h1 style="font-size:22px;margin:0 0 12px">Local service did not start</h1>
      <p style="color:#91a8ba;line-height:1.5">Pulse only connects to the local Token Studio service on 127.0.0.1. It does not use a remote fallback.</p>
...
function createTray() {
  ...
  tray.setToolTip('Token Studio Pulse');
...
async function ensureLocalService() {
  ...
  if (await isTokenStudioApi(existingApi) && await isTokenStudioUi(existingUi) && await isTokenStudioUiApi(existingUi)) {
  ...
  const child = spawn(nodeBin, [
    ...
    env: {
      ...
      TOKEN_STUDIO_LIVE_COLLECT_INTERVAL_SECONDS: String(liveCollectIntervalSeconds())
    },
    stdio: ['ignore', 'pipe', 'pipe'],
    windowsHide: true
  });
  serviceProcess = child;
  child.stdout.on('data', chunk => process.stdout.write(`[token-studio-desktop] ${chunk}`));
  child.stderr.on('data', chunk => process.stderr.write(`[token-studio-desktop] ${chunk}`));
  const api = `http://127.0.0.1:${apiPort}`;
  const ui = `http://127.0.0.1:${uiPort}`;
  await waitForTokenStudio(api, 'local API', isTokenStudioApi);
  await waitForTokenStudio(ui, 'local UI', isTokenStudioUi);
  await waitForTokenStudio(ui, 'local UI API proxy', isTokenStudioUiApi);
  return { api, ui };
}
...
async function isTokenStudioApi(origin) {
  return isTokenStudioLiveEndpoint(`${origin}/api/live`);
}

async function isTokenStudioUiApi(origin) {
  return isTokenStudioLiveEndpoint(`${origin}/api/live`);
}

async function isTokenStudioLiveEndpoint(url) {
  try {
...
async function waitForTokenStudio(origin, label, probe) {
